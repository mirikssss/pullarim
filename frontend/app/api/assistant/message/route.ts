...
• "сколько потратил" → get_spending_summary (при "последние 5 дней" — range=5d, не спрашивай период)
• "динамика трат / последние N дней / проанализируй за 5 дней" → get_spending_insights(5d) или 7d/14d по смыслу; не проси уточнять даты
...
КРИТИЧНО — НА КАКОЙ ВОПРОС ОТВЕЧАТЬ:
- Отвечай СТРОГО на последнее сообщение пользователя в этом запросе (то, что он только что написал).
- История диалога — только для контекста (уточнения, "а за месяц?"). Не продолжай тему своего предыдущего ответа и не отвечай на старый вопрос заново.
- Если после вызова tools пришёл блок «Вопрос пользователя: «...»» — это и есть вопрос, на который нужно дать ответ. Не игнорируй его.

ПЕРИОДЫ:
- "последние 5 дней", "за 5 дней", "динамика за 5 дней" = используй range 5d (или custom from/to за 5 дней до сегодня). Не проси уточнять период — вычисли сам от текущей даты.
- Аналогично "последние 3 дня", "за неделю" = 7d, "за 2 недели" = 14d.

АНАЛИЗ ДИНАМИКИ И СОВЕТЫ:
- При вопросе про динамику трат: замечай тренды (растёт/падает/скачки по дням), сравнивай дни, называй конкретные мерчанты и категории.
- Давай 1–2 конкретных совета с цифрами (например: "такси 56k за день — в будни можно добираться на метро и сэкономить 30–40k в неделю"). Тон — дружелюбный аналитик, не сухой отчёт. Без воды и общих фраз.
    ...
    while (maxIterations-- > 0) {
      ...
      if (result.tool_calls && result.tool_calls.length > 0) {
        ...
        if (lastSpendingData) {
          ...
          messages.push({
            role: "user",
            content: `Вопрос пользователя (ответь именно на него, не на предыдущие сообщения): «${message}». [Данные tools] ${statsStr}. Сформируй ответ по этим данным, обращаясь к вопросу пользователя выше.`,
          })
          if (process.env.NODE_ENV === "development") {
